...
function mostrarSeccion() {
    //Ocultar la seccion con la clase de mostrar
...
function seleccionarFecha() {
    ...
    inputFecha.addEventListener('input', function(e){
        ...
        if([0, 1].includes(dia)) {
            e.target.value = '';
            mostrarAlerta('Domingo y Lunes no abrimos', 'error', '.formulario');
        } else {
...
function mostrarResumen() {
    ...
    if(Object.values(cita).includes('') || cita.servicios.length === 0) {
        mostrarAlerta('Faltan datos para hacer la reserva', 'error', '.contenido-resumen', false);
